        ...
        <div class="row justify-content-center">
            <div class="col-lg-6">
                <div style="margin: 60px">
                    @if($header->member_id)
                    <h3><u id="jsCustomerData">{{$header->member_id.' - '.$header->customer_name.' #'.$header->id}}</u></h3>
                    @else
                    <h3><u id="jsCustomerData">{{'Guest - '.$header->customer_name.' #'.$header->id}}</u></h3>
                    @endif
                    <b>Pilih Item Series</b>
                    <div>
                        <input id="jsSearchSeriesItemKeyword" type="text" class="form-control js-clear-selected" name="keyword" value="" placeholder="Keyword: Nama Item / Item Code" />
...
<input type="hidden" id="jsViewedOngoingTransId" value="{{$header->id}}" />
<input type="hidden" id="jsUrlGetSeriesItemsAutoComplete" value="{{route('cashier.searches-series-items', array('trans_id', request()->branch_id))}}" />
...
<script src="{{ URL::asset('js/cashier-rev/series-item-auto-complete.js') }}" type="text/javascript"></script>
